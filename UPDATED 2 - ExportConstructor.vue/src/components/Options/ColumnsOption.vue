<template>
                ...
                <div class="pos-a flex columns">
                    <vuedraggable :list="nextElement?.value" class="flex" animation="250">
                        <div v-for="element in nextElement.value" :key="element">
                            <div v-if="element.selected != 'hide'" class="acc__item mr-1 pos-r" style="min-width: 100px;
    ...
    align-items: center;" :class="{
        gray: !element.selected,
                                ...
                                <input type="checkbox" class="ml-1" v-if="element.selected != 'default'"
                                ...
                                <div class="pos-a pl-0 w-full" style="top:100%;left:0;padding-top: 4px"
                                    ...
                                    <template v-for="param in element.params" :key="param.name">
                                        <div class="acc__item m-0 mb-0.5 flex" v-if="param.type == 'checkbox'" :class="{
                                            gray: !param.selected
                                        }">
                                            <span>{{ param.name }}</span>
                                            <input type="checkbox" class="ml-auto" v-model="param.selected" />
                                        </div>
