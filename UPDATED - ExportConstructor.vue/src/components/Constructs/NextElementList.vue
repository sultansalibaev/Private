...
export default {
    ...
    methods: {
        select(elementOption, keyName, isSelect = false) {
            elementOption.forEach((el) => {
                if (isSelect) {
                    el.selected = el.name == keyName;
                } else {
                    el[0].selected = el[0].name == keyName;
                }
            });
        },
    ...
    mounted() {
        ...
        table_titles.forEach((table_title) => {
            let columns = table_title.nextElementSibling.querySelector(
                ".pos-a.flex.columns"
            );
            columns.style.left = `${-table_title.clientWidth}px`;
        });
    },
};
</script>
