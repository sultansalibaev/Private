                ...
                <construct-button
                    :class="
                        option.position == 'center'
                            ...
                            : option.type + ' element__parent'
                    "
                    :style="option.value == 'text_length' && !option?.selected ? 'background:#ccc;' : `background:${elementColor};${option.valueRU == 'Порядок' ? 'padding: 0 34px 0 34px' : ''}`"
                    v-else
                    @click="dropDown($event, option, option.selected)"
                    :id="option.id"
                    :title="option.description ? option.description : ''"
...
export default {
    ...
    methods: {
        ...
        dropDown(ev, option, selected) {
            if (option != null && selected != null && option?.value == "text_length") {
                option.selected = this.myLog(selected);
                if (ev.target.getAttribute('type') == 'checkbox') {
                    let dropDown = ev.target.parentElement.nextElementSibling;
                    dropDown.style.display = "none";
                    dropDown.classList.add("hidden");
                    ev.target.parentElement.style.borderRadius = "5px";
                    this.setElementHeight(ev, false);
                }
                if (!option.selected) return this.hideOther('selected');
            }
