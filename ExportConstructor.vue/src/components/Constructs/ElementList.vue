<template>
    <vuedraggable
        ...
        handle=".handle"
    >
        <div v-for="[key, value] of getReportCopy()" :key="key.id">
            <div class="group" v-if="key.name === 'GROUP'">
                ...
                <div class="group__title group-bg flex items-center" v-else>
                    ...
                    <button class="remove-element" @click="removeProject(key.id)">
                        Удалить
                    </button>
                </div>
...
import { removeGroup } from "@/hooks/toggle/group";
import { removeProject } from "@/hooks/toggle/project";
